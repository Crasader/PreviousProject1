...
void ChongMingView::startGameAgain(){
    if(getChildByTag(2000)!=NULL){
        getChildByTag(2000)->removeFromParent();
    }
...
void ChongMingView::updatePlayerView(int type, Player* playerInfo){
    if (GAMEDATA::getInstance()->getMahjongRoomType()==MahjongRoom::privateRoom){
...
void ChongMingView::addPlayer2Room(){
    vector<Player*> players = GAMEDATA::getInstance()->getPlayersInfo();
    for (int i = 0; i < players.size(); i++){
...
void ChongMingView::checkPlayerIpRepetition(){
    vector<Player*> players = GAMEDATA::getInstance()->getPlayersInfo();
    for(int i=0;i<players.size();i++){
...
int  ChongMingView::getNumbersByPoker(string pokers){
...
void ChongMingView::drawCpgControllPad(PlayerCpgtData newData){
    shmjHeroCpgtData.playCpgt = newData;
    controllPad->removeAllChildrenWithCleanup(true);
    auto qi = MenuItemImage::create("gameview/mj_qi.png", "gameview/mj_qi.png", CC_CALLBACK_0(ChongMingView::heroDoCpgQi,this));
    ...
    if (newData.hu == 1){
        hu = MenuItemImage::create("gameview/mj_hu.png", "gameview/mj_hu.png", CC_CALLBACK_0(ChongMingView::playerApplyHu, this));
    ...
    if (newData.peng != ""){
        peng = MenuItemImage::create("gameview/mj_peng.png", "gameview/mj_peng.png", CC_CALLBACK_1(ChongMingView::heroDoPeng, this));
...
void ChongMingView::showGuiLayer(){
    guiLayer->setVisible(true);
}

void ChongMingView::removeHeroPlayedIcon(){
    if(NULL != playerHero)
        playerHero->removePlayedIcon();
}


void ChongMingView::showHeroChiUi(Ref* ref){
    controllPad->setVisible(false);
    //吃牌排序
    if ( shmjHeroCpgtData.playCpgt.chi[0] != "" &&  shmjHeroCpgtData.playCpgt.chi[1] != ""){
        ...
        for (int i = 0; i < 3; i++){
            ...
            for (int j = 0; j < pai.size(); j++){
                ...
                MenuItemImage* imageItem = MenuItemImage::create(imageName, imageName, CC_CALLBACK_1(ChongMingView::heroDoChi, this));
                imageItem->setTag(i);
...
void ChongMingView::showHeroGangUi(Ref* ref){
    controllPad->setVisible(false);
    ...
    if (allGangs.size()>1){
        ...
        for (int i = 0; i < allGangs.size(); i++){
            ...
            for (int l = 0; l < 4; l++){
                ...
                MenuItemImage* imageItem = MenuItemImage::create(imageName, imageName, CC_CALLBACK_1(ChongMingView::heroDoGang, this));
...
void ChongMingView::heroDoChi(Ref* psend){
    if (NULL != choiceMenu){
        choiceMenu->setVisible(false);
        choiceMenu->removeAllChildren();
    }
...
void ChongMingView::setCurrentJongVisible(int seatId){
    //TODO
}

void ChongMingView::playerTingAnim(int seatId){
...
void ChongMingView::clearRoomPlayer(){
...
void ChongMingView::recoverGame(){
    if(getChildByTag(2000)!=NULL){
        getChildByTag(2000)->removeFromParent();
    }
    //重绘制场景
...
void ChongMingView::recoverPlayer(PlayerGameData data, int type, Player* playerInfo){
    if (type == ClientSeatId::hero){
        if (playerHero == NULL){
            playerHero = PlayerHero::create();
            playerHero->initPlayer(playerInfo);
...
//显示玩家的方向和庄
void ChongMingView::showOriention(){
...
PlayerBase* ChongMingView::getPlayerBySeatId(int sid){
...
void ChongMingView::firstReplaceFlower(ReplaceJongVec vec,PlayerCpgtData data) {
...
void ChongMingView::showPaiduiNum(int num){
...
void ChongMingView::dealJongFinish(ReplaceJongVec vec,PlayerCpgtData data){
...
void ChongMingView::showHuPaiXing(std::string paixing){
    if(paixing==""){
        return;
    }
    float posY = 260;
...
void ChongMingView::showHandPokerOver(int seatId){
    vector<GameResultData> results = GAMEDATA::getInstance()->getGameResults();
    string leftJongs="";
    string rightJongs="";
...
void ChongMingView::onEnterTransitionDidFinish(){
    scheduleUpdate();
    if(GAMEDATA::getInstance()->getContinueAgain()){
...
void ChongMingView::onExit()
{
    ...
    Director::getInstance()->getEventDispatcher()->removeEventListener(dissovleRoomNew);
