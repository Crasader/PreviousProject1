...
#include "mahjong/result/baida/special/BDGameResultCell.hpp"
...
bool BDSpecialResultLayer::init(){
    if(!Layer::init()){
        return false;
    }
...
void BDSpecialResultLayer::showResultTitle(){
...
void BDSpecialResultLayer::showGameReslut(){
    ...
    for (int i=0; i<results.size();i++) {
        BDGameResultCell* cell = BDGameResultCell::create(results.at(i));
...
void BDSpecialResultLayer::shareResult(){
    ...
    utils::captureScreen(CC_CALLBACK_2(BDSpecialResultLayer::afterCaptured, this) ,path);
#endif
...
void BDSpecialResultLayer::afterCaptured(bool succeed, const std::string &outputFile)
{
    if (succeed) {
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
...
void BDSpecialResultLayer::onEnter(){
    Layer::onEnter();
...
void BDSpecialResultLayer::onExit(){
    Layer::onExit();
