...
bool SZSpecialResultLayer::init(){
    if(!Layer::init()){
        return false;
    }
...
void SZSpecialResultLayer::showResultTitle(){
...
void SZSpecialResultLayer::showGameReslut(){
    ...
    for (int i=0; i<results.size();i++) {
        SZGameResultCell* cell = SZGameResultCell::create(results.at(i));
...
void SZSpecialResultLayer::shareResult(){
    ...
    utils::captureScreen(CC_CALLBACK_2(SZSpecialResultLayer::afterCaptured, this) ,path);
#endif
...
void SZSpecialResultLayer::afterCaptured(bool succeed, const std::string &outputFile)
{
    if (succeed) {
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
...
void SZSpecialResultLayer::onEnter(){
    Layer::onEnter();
...
void SZSpecialResultLayer::onExit(){
    Layer::onExit();
