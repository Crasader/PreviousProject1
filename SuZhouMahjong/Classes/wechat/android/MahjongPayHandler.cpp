...
void MahjongPayHandler::loginThirdPlatform(std::string openid,std::string unionid, std::string url,
                                           std::string sex, std::string nickname,std::string hsman,std::string hstype,std::string imsi,std::string imei,std::string ver1,std::string jd,std::string wd) {
    if ("" == openid) {
        //登录错误的提示
        ...
        UserData::getInstance()->setLongitude(jd);
        UserData::getInstance()->setLatitude(wd);
        std::string msg = CommandManage::getInstance()->getThirdLoginCommand(
                                                                             openid,unionid, url,sex,nickname,hsman,hstype,imsi,imei,ver1,jd,wd);
        if (msg != "") {
...
void MahjongPayHandler::addRoomByInvited(std::string roomid){
