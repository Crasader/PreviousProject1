...
#include "mahjong/result/chongming/special/CMGameResultCell.hpp"
...
bool CMSpecialResultLayer::init(){
    if(!Layer::init()){
        return false;
    }
...
void CMSpecialResultLayer::showResultTitle(){
...
void CMSpecialResultLayer::showGameReslut(){
    ...
    for (int i=0; i<results.size();i++) {
        CMGameResultCell* cell = CMGameResultCell::create(results.at(i));
...
void CMSpecialResultLayer::shareResult(){
    ...
    utils::captureScreen(CC_CALLBACK_2(CMSpecialResultLayer::afterCaptured, this) ,path);
#endif
...
void CMSpecialResultLayer::afterCaptured(bool succeed, const std::string &outputFile)
{
    if (succeed) {
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
...
void CMSpecialResultLayer::onEnter(){
    Layer::onEnter();
...
void CMSpecialResultLayer::onExit(){
    Layer::onExit();
