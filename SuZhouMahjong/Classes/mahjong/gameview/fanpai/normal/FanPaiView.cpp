...
void FanPaiView::initFanPaiMahjong(){
...
void FanPaiView::checkHeroHandPoker(){
    if(NULL != playerHero)
        playerHero->checkFanPaiHeroHandPoker(atoi(GAMEDATA::getInstance()->getBaiDaPoker().c_str()));
}

void FanPaiView::dealJongFinish(MahjongFaPaiData data){
...
void FanPaiView::showBaiDaControllPad(PlayerCpgtData newData){
    ...
    int buttonCount = 0;
    if (newData.hu == 1){
        hu = MenuItemImage::create("gameview/mj_hu.png", "gameview/mj_hu.png", CC_CALLBACK_1(FanPaiView::doBaiDaHuAction, this));
        hu->setPosition(Point(-buttonCount * 160+10, 0));
        controllPad->addChild(hu);
        buttonCount++;
    }
    if (newData.playerGang.size()>0){
        gang = MenuItemImage::create("gameview/mj_gang.png", "gameview/mj_gang.png", CC_CALLBACK_1(FanPaiView::showBaiDaGangChoice, this));
    ...
    if (newData.peng != ""){
        peng = MenuItemImage::create("gameview/mj_peng.png", "gameview/mj_peng.png", CC_CALLBACK_1(FanPaiView::doBaiDaPengAction, this));
    ...
    auto qi = MenuItemImage::create("gameview/mj_qi.png", "gameview/mj_qi.png", CC_CALLBACK_1(FanPaiView::doBaiDaQiAction,this));
    qi->setPosition(Point(-buttonCount * 160, 0));
    controllPad->addChild(qi);
    controllPad->setVisible(true);
}

void FanPaiView::hideBaiDaControllPad(){
...
void FanPaiView::doBaiDaQiAction(Ref* ref){
    ...
    NetworkManage::getInstance()->sendMsg(CommandManage::getInstance()->getFPGiveUpCommand());
}

void FanPaiView::doBaiDaHuAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
    NetworkManage::getInstance()->sendMsg(CommandManage::getInstance()->getFPHuActionCommand("1"));
}


void FanPaiView::doBaiDaPengAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
    MenuItemImage* temp = (MenuItemImage*)ref;
    NetworkManage::getInstance()->sendMsg(CommandManage::getInstance()->getFPPengCommand(temp->getName(), temp->getTag()));
}

void FanPaiView::showBaiDaGangChoice(Ref* ref){
    controllPad->setVisible(false);
    MenuItemImage* temp = (MenuItemImage*)ref;
    ...
    if (allGangs.size()>1){
        ...
        for (int i = 0; i < allGangs.size(); i++){
            ...
            for (int l = 0; l < 4; l++){
                ...
                MenuItemImage* imageItem = MenuItemImage::create(imageName, imageName, CC_CALLBACK_1(FanPaiView::doBaiDaGangAction, this));
...
void FanPaiView::doBaiDaGangAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
    ...
    NetworkManage::getInstance()->sendMsg(CommandManage::getInstance()->getFPGangCommand(StringUtils::format("%d",tag),tag,gangData.flag));
...
void FanPaiView::doRecoverPlayer(PlayerGameData data, int type, PlayerInfo* playerInfo){
...
void FanPaiView::showHuPaiXing(std::string paixing){
    if(paixing==""){
        return;
    }
...
void FanPaiView::showHandPokerOver(int seatId){
    vector<GameResultData> results = GAMEDATA::getInstance()->getGameResults();
    string leftJongs="";
    string rightJongs="";
...
void FanPaiView::onEnterTransitionDidFinish(){
...
void FanPaiView::onEnter(){
    BaseView::onEnter();
    faPaiNotifyListener = Director::getInstance()->getEventDispatcher()->addCustomEventListener(MSG_GAME_FP_START_FAPAI_NOTIFY, [=](EventCustom* event){
