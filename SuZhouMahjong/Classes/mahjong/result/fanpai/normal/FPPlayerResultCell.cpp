...
FPPlayerResultCell* FPPlayerResultCell::create(GameResultData data){
    FPPlayerResultCell* cell = new FPPlayerResultCell();
...
bool FPPlayerResultCell::init(GameResultData data){
    if(!Sprite::init()){
        return false;
    }
    ...
    // 从新排序
    std::vector<std::string> baida;
    
    vector<std::string>::iterator baidaItor;
    for(baidaItor=showPokers.begin();baidaItor!=showPokers.end();)
    {
        if(GAMEDATA::getInstance()->getBaiDaPoker()==*baidaItor)
        {
            baida.push_back(*baidaItor);
            baidaItor=showPokers.erase(baidaItor);
            break;
        }
        else
        {
    ...
    for(int i=0;i<finalShowPoker.size();i++){
        for (int j = 0; j < finalShowPoker.at(i).size(); j++){
            ...
            if(GAMEDATA::getInstance()->getBaiDaPoker() == finalShowPoker.at(i).at(j)){
                jong->showBaiDaPoker();
            }
