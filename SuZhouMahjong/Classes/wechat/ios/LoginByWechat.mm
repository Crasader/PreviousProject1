...
-(void) payWeChat:(NSString*) poxiaoId PayPoint:(NSString*) payPoint{
    NSString* urlString= [NSString stringWithFormat:@"%s?charge_type=1&tbu_id=201710&pay_platform=apple&game_version=1&hsman=ios&hstype=ios&imei=123456789&imsi=46000&channel_id=apple&request_pay_amount=1&poxiao_id=%@&pay_point=%@",APP_WECHAT_PAY,poxiaoId,payPoint];
    NSLog(@"url:%@",urlString);
    //解析服务端返回json数据
    //加载一个NSURL对象
    ...
    NSString *result = [[NSString alloc]initWithData:received encoding:NSUTF8StringEncoding];
    NSLog(@"received = %@",result);
    Application::getInstance()->openURL(std::string([result UTF8String]));
}
