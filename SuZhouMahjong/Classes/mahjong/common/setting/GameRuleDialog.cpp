...
bool GameRuleDialog::init(){
    ...
    shangHai = Sprite::create("openroom/suzhou_btn_1.png");
    ...
    chongMing =  Sprite::create("openroom/baida_btn_2.png");
    chongMing->setPosition(225,325);
    addChild(chongMing);
    
    fanPai =  Sprite::create("openroom/fanpaibaida_btn_2.png");
    fanPai->setPosition(225,220);
    addChild(fanPai);
    ...
    auto text = Sprite::create("rule/rule_sz_1.png");
...
void GameRuleDialog::onTouchEnded(Touch *touch, Event  *event){
    if(shangHai->getBoundingBox().containsPoint(touch->getLocation())){
        ruleSelected = 0;
        shangHai->setTexture("openroom/suzhou_btn_1.png");
        hongZhong->setTexture("openroom/hong_zhong_btn_2.png");
        chongMing->setTexture("openroom/baida_btn_2.png");
        fanPai->setTexture("openroom/fanpaibaida_btn_2.png");
        updateRuleImage(ruleSelected,selcetIndex);
    }
    if(hongZhong->getBoundingBox().containsPoint(touch->getLocation())){
        ruleSelected = 1;
        shangHai->setTexture("openroom/suzhou_btn_2.png");
    ...
    if(chongMing->getBoundingBox().containsPoint(touch->getLocation())){
        ...
        shangHai->setTexture("openroom/suzhou_btn_2.png");
        hongZhong->setTexture("openroom/hong_zhong_btn_2.png");
        chongMing->setTexture("openroom/baida_btn_1.png");
        fanPai->setTexture("openroom/fanpaibaida_btn_2.png");
        updateRuleImage(ruleSelected,selcetIndex);
    }
    if(fanPai->getBoundingBox().containsPoint(touch->getLocation())){
        ruleSelected = 3;
        shangHai->setTexture("openroom/suzhou_btn_2.png");
        hongZhong->setTexture("openroom/hong_zhong_btn_2.png");
        chongMing->setTexture("openroom/baida_btn_2.png");
        fanPai->setTexture("openroom/fanpaibaida_btn_1.png");
        updateRuleImage(ruleSelected,selcetIndex);
    }
}
...
void GameRuleDialog::updateRuleImage(int indexSelect,int index){
    ...
    if(indexSelect == 0){
        text->setTexture(StringUtils::format("rule/rule_sz_%d.png",index));
        ...
        if(index == 4){
            ...
            text->setTexture(StringUtils::format("rule/rule_bd_%d.png",index));
        }
        
    }else if(indexSelect == 3){
        text->setTexture(StringUtils::format("rule/rule_fp_%d.png",index));
    }
    text->setPosition(435,(text->getContentSize().height+50)/2);
    Layout *customItem = Layout::create();
    customItem->setLayoutType(Layout::Type::ABSOLUTE);
