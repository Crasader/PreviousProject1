...
void SuZhouView::initSuZhouMahjong(){
...
void SuZhouView::checkHeroHandPoker(){
    if(NULL != playerHero)
        playerHero->checkSuZhouHeroHandPoker();
}

void SuZhouView::dealJongFinish(MahjongFaPaiData data){
    BaseView::dealJongFinish(data);
    GAMEDATA::getInstance()->setPrivateDLNum(data.diling);
    GAMEDATA::getInstance()->setPrivateHFNum(data.huangfan);
    updateRoomInfo();
    ...
    if(GAMEDATA::getInstance()->getCurrentBank() == GAMEDATA::getInstance()->getHeroSeatId()){
        ...
        if(data.mjHuData.hu == 1||data.hasGang){
            showSuZHouControllPad(data.mjHuData);
...
void SuZhouView::showSuZHouControllPad(PlayerCpgtData newData){
    ...
    int buttonCount = 0;
    if (newData.hu == 1){
        hu = MenuItemImage::create("gameview/mj_hu.png", "gameview/mj_hu.png", CC_CALLBACK_1(SuZhouView::doSuZhouHuAction, this));
        hu->setPosition(Point(-buttonCount * 160+10, 0));
        controllPad->addChild(hu);
        buttonCount++;
    }
    if (newData.playerGang.size()>0){
        gang = MenuItemImage::create("gameview/mj_gang.png", "gameview/mj_gang.png", CC_CALLBACK_1(SuZhouView::showSuZhouGangChoice, this));
    ...
    if (newData.peng != ""){
        peng = MenuItemImage::create("gameview/mj_peng.png", "gameview/mj_peng.png", CC_CALLBACK_1(SuZhouView::doSuZhouPengAction, this));
    ...
    auto qi = MenuItemImage::create("gameview/mj_qi.png", "gameview/mj_qi.png", CC_CALLBACK_1(SuZhouView::doSuZhouQiAction,this));
    qi->setPosition(Point(-buttonCount * 160, 0));
    controllPad->addChild(qi);
    controllPad->setVisible(true);
}

void SuZhouView::hideSuZHouControllPad(){
...
void SuZhouView::doSuZhouQiAction(Ref* ref){
...
void SuZhouView::doSuZhouHuAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void SuZhouView::doSuZhouPengAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void SuZhouView::showSuZhouGangChoice(Ref* ref){
    controllPad->setVisible(false);
    MenuItemImage* temp = (MenuItemImage*)ref;
    ...
    if (allGangs.size()>1){
        ...
        for (int i = 0; i < allGangs.size(); i++){
            ...
            for (int l = 0; l < 4; l++){
                ...
                MenuItemImage* imageItem = MenuItemImage::create(imageName, imageName, CC_CALLBACK_1(SuZhouView::doSuZhouGangAction, this));
...
void SuZhouView::doSuZhouGangAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void SuZhouView::doRecoverPlayer(PlayerGameData data, int type, PlayerInfo* playerInfo){
...
void SuZhouView::showHuPaiXing(std::string paixing){
    if(paixing==""){
        return;
    }
...
void SuZhouView::showHandPokerOver(int seatId){
    vector<GameResultData> results = GAMEDATA::getInstance()->getGameResults();
    string leftJongs="";
    string rightJongs="";
...
void SuZhouView::onEnterTransitionDidFinish(){
...
void SuZhouView::onEnter(){
    BaseView::onEnter();
    
    //苏州麻将发牌
    faPaiNotifyListener = Director::getInstance()->getEventDispatcher()->addCustomEventListener(MSG_GAME_START_FAPAI_NOTIFY, [=](EventCustom* event){
