...
YIMErrorcode MyIM::login(std::string username, std::string password)
{
	...
	{
		log("call Login()  success");
	}
	else
	{
		log("call Login fail,errorcode is %d", ymErrorcode);
	}
	return ymErrorcode;
}
YIMErrorcode MyIM::joinRoom(std::string roomid)
{
	...
	{
		log("joinRoom()  fail,errorcode is %d", (int)ymErrorcode);
	}
	return ymErrorcode;
}
YIMErrorcode MyIM::leaveRoom(std::string roomid)
{
	...
	if (ymErrorcode != YIMErrorcode_Success)
	{
		log("leaveRoom()  fail,errorcode is %d", (int)ymErrorcode);
	}
	return ymErrorcode;
}
...
void MyIM::OnRecvMessage(IYIMMessage* pMessage)
{
	 ...
	 if (msgType == YIMMessageBodyType::MessageBodyType_Voice)
	{
		...
		log("RecvMessage text is %s ,messageid is %llu ,param is %s ,audio time is %d",
			pMsgVoice->GetText(),
...
{
	log("OnDownload ,messageid is %llu ,errorcode is %d ,savePath is %s",
		messageID,
		(int)errorcode,
...
{
	log("[YOUMI] MyIM::OnSendAudioMessageStatus requestID=%d,errorcode=%d,audioPath=%s,audioTime=%d", (int)requestID, (int)errorcode, XcharToString(audioPath).c_str(), audioTime);
}
