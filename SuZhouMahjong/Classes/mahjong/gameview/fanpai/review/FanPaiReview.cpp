...
void FanPaiReview::initBaiDaMahjong(){
    GAMEDATA::getInstance()->setIsPlaying(false);
    GAMEDATA::getInstance()->setIsLiuJu(false);
    GAMEDATA::getInstance()->setIsGotoLobby(false);
    
    auto gameName = Sprite::create("gameview/fp_baidamajiang.png");
    gameName->setPosition(640,290);
    addChild(gameName);
    ...
    difen->setTexture(StringUtils::format("gameview/zi_di_%s.png",GAMEDATA::getInstance()->getFPDiFen().c_str()));
    addChild(difen);
    auto menQing  = Sprite::create("gameview/zi_youmenqing.png");
    menQing->setAnchorPoint(Point::ANCHOR_MIDDLE_LEFT);
    addChild(menQing);
    if(GAMEDATA::getInstance()->getFPMenQing() == "1"){
        menQing->setVisible(true);
    }else{
        menQing->setVisible(false);
    }
    int wid = payModle->getContentSize().width+(menQing->isVisible()?menQing->getContentSize().width:0) + difen->getContentSize().width;
    payModle->setPosition(640-wid/2,160);
    difen->setPosition(640-wid/2+payModle->getContentSize().width,160);
    menQing->setPosition(640-wid/2+difen->getContentSize().width+payModle->getContentSize().width,160);
}

void FanPaiReview::checkHeroHandPoker(){
    if(NULL != playerHero)
        playerHero->checkFanPaiHeroHandPoker(atoi(GAMEDATA::getInstance()->getBaiDaPoker().c_str()));
}

void FanPaiReview::dealJongFinish(MahjongFaPaiData data){
    ...
    //判断是或自己出牌显示翻的牌并锁住百搭牌
    auto fanbg = Sprite::create("gameview/fan_pai_bg.png");
    fanbg->setPosition(760,410);
    addChild(fanbg);
    Jong* myJong = Jong::create();
    myJong->showJong(heroplayed, atoi(GAMEDATA::getInstance()->getFanPaiPoker().c_str()));
    myJong->setPosition(760,400);
    myJong->setScale(1.2f);
    addChild(myJong);
...
void FanPaiReview::showBaiDaControllPad(PlayerCpgtData newData){
    controllPad->removeAllChildrenWithCleanup(true);
    auto qi = MenuItemImage::create("gameview/mj_qi.png", "gameview/mj_qi.png", CC_CALLBACK_1(FanPaiReview::doBaiDaQiAction,this));
    ...
    if (newData.hu == 1){
        hu = MenuItemImage::create("gameview/mj_hu.png", "gameview/mj_hu.png", CC_CALLBACK_1(FanPaiReview::doBaiDaHuAction, this));
    ...
    if (newData.peng != ""){
        peng = MenuItemImage::create("gameview/mj_peng.png", "gameview/mj_peng.png", CC_CALLBACK_1(FanPaiReview::doBaiDaPengAction, this));
    ...
    if (newData.playerGang.size()>0){
        ...
        gang = MenuItemImage::create("gameview/mj_gang.png", "gameview/mj_gang.png", CC_CALLBACK_1(FanPaiReview::showBaiDaGangChoice, this));
...
void FanPaiReview::hideBaiDaControllPad(){
...
void FanPaiReview::doBaiDaPengAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void FanPaiReview::showBaiDaGangChoice(Ref* ref){
    controllPad->setVisible(false);
    MenuItemImage* temp = (MenuItemImage*)ref;
    ...
    if (allGangs.size()>1){
        ...
        for (int i = 0; i < allGangs.size(); i++){
            ...
            for (int l = 0; l < 4; l++){
                ...
                MenuItemImage* imageItem = MenuItemImage::create(imageName, imageName, CC_CALLBACK_1(FanPaiReview::doBaiDaGangAction, this));
...
void FanPaiReview::doBaiDaGangAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void FanPaiReview::doRecoverGame(){
    //重绘制场景
...
void FanPaiReview::doRecoverPlayer(PlayerGameData data, int type, PlayerInfo* playerInfo){
...
void FanPaiReview::controlPause(){
    playing= !playing;
...
void FanPaiReview::update(float dt){
    interval += dt;
    if(playing){
        if(interval>1.2){
            if(fupanStep<GAMEDATA::getInstance()->getPlaybackInfo().playBackInfo.size()){
...
void FanPaiReview::showHuPaiXing(std::string paixing){
    if(paixing==""){
        return;
    }
...
void FanPaiReview::showHandPokerOver(int seatId){
    vector<GameResultData> results = GAMEDATA::getInstance()->getGameResults();
    string leftJongs="";
    string rightJongs="";
...
void FanPaiReview::onEnterTransitionDidFinish(){
    scheduleUpdate();
...
void FanPaiReview::onEnter(){
    BaseView::onEnter();
    //断线续玩
    conncetAgainListener=  Director::getInstance()->getEventDispatcher()->addCustomEventListener(MSG_PLAYER_FP_CONNECT_AGAIN, [=](EventCustom* event){
        //重新绘制玩家的牌和话
    ...
    //好友房间游戏未开始重新连接
    openFriendRoomListener = Director::getInstance()->getEventDispatcher()->addCustomEventListener(MSG_FP_FRIEND_OPEN_ROOM_RESP, [=](EventCustom* event){
    ...
    enterFriendRoomListener=  Director::getInstance()->getEventDispatcher()->addCustomEventListener(MSG_FP_ENTER_FRIEND_ROOM_RESP, [=](EventCustom* event){
    ...
    turnListener = EventListenerCustom::create(MSG_FP_PLAYER_TURN_WHO, [=](EventCustom* event){
        //收到出牌通知,手里没牌
    ...
    fupanPlayerInfoListener = EventListenerCustom::create(MSG_GAME_FU_PAN_PLAYER_NOTIFY, [=](EventCustom* event){
        ...
        image1= MenuItemImage::create("fupan/down_1.png", "fupan/down_2.png",CC_CALLBACK_0(FanPaiReview::controlDown, this));
        auto image2 = MenuItemImage::create("fupan/pause_1.png", "fupan/pause_2.png",CC_CALLBACK_0(FanPaiReview::controlPause, this));
        image2->setTag(1087);
        image3 = MenuItemImage::create("fupan/up_1.png", "fupan/up_2.png","fupan/up_3.png",CC_CALLBACK_0(FanPaiReview::controlUp, this));
        MenuItemImage* image4 = MenuItemImage::create("fupan/back_1.png", "fupan/back_2.png",CC_CALLBACK_0(FanPaiReview::controlBack, this));
