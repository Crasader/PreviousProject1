...
void BaiDaReview::initBaiDaMahjong(){
...
void BaiDaReview::checkHeroHandPoker(){
    if(NULL != playerHero)
        playerHero->checkBaiDaHeroHandPoker();
}

void BaiDaReview::dealJongFinish(MahjongFaPaiData data){
...
void BaiDaReview::showBaiDaControllPad(PlayerCpgtData newData){
    controllPad->removeAllChildrenWithCleanup(true);
    auto qi = MenuItemImage::create("gameview/mj_qi.png", "gameview/mj_qi.png", CC_CALLBACK_1(BaiDaReview::doBaiDaQiAction,this));
    ...
    if (newData.hu == 1){
        hu = MenuItemImage::create("gameview/mj_hu.png", "gameview/mj_hu.png", CC_CALLBACK_1(BaiDaReview::doBaiDaHuAction, this));
    ...
    if (newData.peng != ""){
        peng = MenuItemImage::create("gameview/mj_peng.png", "gameview/mj_peng.png", CC_CALLBACK_1(BaiDaReview::doBaiDaPengAction, this));
    ...
    if (newData.playerGang.size()>0){
        ...
        gang = MenuItemImage::create("gameview/mj_gang.png", "gameview/mj_gang.png", CC_CALLBACK_1(BaiDaReview::showBaiDaGangChoice, this));
...
void BaiDaReview::hideBaiDaControllPad(){
...
void BaiDaReview::doBaiDaPengAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void BaiDaReview::showBaiDaGangChoice(Ref* ref){
    controllPad->setVisible(false);
    MenuItemImage* temp = (MenuItemImage*)ref;
    ...
    if (allGangs.size()>1){
        ...
        for (int i = 0; i < allGangs.size(); i++){
            ...
            for (int l = 0; l < 4; l++){
                ...
                MenuItemImage* imageItem = MenuItemImage::create(imageName, imageName, CC_CALLBACK_1(BaiDaReview::doBaiDaGangAction, this));
...
void BaiDaReview::doBaiDaGangAction(Ref* ref){
    controllPad->setVisible(false);
    playerHero->stopTimeClockAnim();
...
void BaiDaReview::doRecoverGame(){
    //重绘制场景
...
void BaiDaReview::doRecoverPlayer(PlayerGameData data, int type, PlayerInfo* playerInfo){
...
void BaiDaReview::update(float dt){
    interval += dt;
    if(playing){
        if(interval>1.2){
            if(fupanStep<GAMEDATA::getInstance()->getPlaybackInfo().playBackInfo.size()){
...
void BaiDaReview::showHuPaiXing(std::string paixing){
    if(paixing==""){
        return;
    }
...
void BaiDaReview::showHandPokerOver(int seatId){
    vector<GameResultData> results = GAMEDATA::getInstance()->getGameResults();
    string leftJongs="";
    string rightJongs="";
...
void BaiDaReview::onEnterTransitionDidFinish(){
    scheduleUpdate();
...
void BaiDaReview::onEnter(){
    ...
    fupanPlayerInfoListener = EventListenerCustom::create(MSG_GAME_FU_PAN_PLAYER_NOTIFY, [=](EventCustom* event){
        ...
        image1= MenuItemImage::create("fupan/down_1.png", "fupan/down_2.png",CC_CALLBACK_0(BaiDaReview::controlDown, this));
        auto image2 = MenuItemImage::create("fupan/pause_1.png", "fupan/pause_2.png",CC_CALLBACK_0(BaiDaReview::controlPause, this));
        image2->setTag(1087);
        image3 = MenuItemImage::create("fupan/up_1.png", "fupan/up_2.png","fupan/up_3.png",CC_CALLBACK_0(BaiDaReview::controlUp, this));
        MenuItemImage* image4 = MenuItemImage::create("fupan/back_1.png", "fupan/back_2.png",CC_CALLBACK_0(BaiDaReview::controlBack, this));
