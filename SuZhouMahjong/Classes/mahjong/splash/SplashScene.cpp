...
//#include "baidu/ios/BaiDuLocationAPI.h"
...
void SplashScene::onEnterTransitionDidFinish(){
    ...
    if(UserData::getInstance()->getWxOpenId()!= "unknow"){
        ...
        schedule(schedule_selector(SplashScene::updateCount), 0, 0, 6);
    }
...
void SplashScene::loginByWechat(){
    ...
    //oTIvfwn1BAU-wWCZjKmRVlmwYHqA zuo
    NetworkManage::getInstance()->connect();
    NetworkManage::getInstance()->sendMsg(CommandManage::getInstance()->getThirdLoginCommand("oTIvfwn1BAU-wWCZjKmRVlmwYHqA","1131111","http://wx.qlogo.cn/mmopen/iaS020Z6hznYwWiacdX0aia7ia9XANXWGKReDZYCjSM8Jt1MFqtnaPRL4ugpZK8cU2bhVmgHs24KB3LDicrQ1cxjeQngXmburObUM/0","1","Poli","APPLE","iphone","11111111111","11111111111","1.0.4",UserData::getInstance()->getLongitude(),UserData::getInstance()->getLatitude()));
#endif
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
...
void SplashScene::onEnter(){
    ...
    fpReEnterFriendRoomListener  =  Director::getInstance()->getEventDispatcher()->addCustomEventListener(MSG_FP_ENTER_FRIEND_ROOM_RESP, [=](EventCustom* event){
        int result = GAMEDATA::getInstance()->getFriendOpenRoomResp().result;
        if (result == 1){
            NetworkManage::getInstance()->startSocketBeat(CommandManage::getInstance()->getHeartCommmand());
            GAMEDATA::getInstance()->setMahjongRoomType(MahjongRoom::privateRoom);
...
void SplashScene::onExit(){
    ...
    Director::getInstance()->getEventDispatcher()->removeEventListener(fpReEnterFriendRoomListener);
