...
#include "mahjong/result/fanpai/special/FPGameResultCell.hpp"
...
bool FPSpecialResultLayer::init(){
    if(!Layer::init()){
        return false;
    }
...
void FPSpecialResultLayer::showResultTitle(){
...
void FPSpecialResultLayer::showGameReslut(){
    ...
    for (int i=0; i<results.size();i++) {
        FPGameResultCell* cell = FPGameResultCell::create(results.at(i));
...
void FPSpecialResultLayer::shareResult(){
    ...
    utils::captureScreen(CC_CALLBACK_2(FPSpecialResultLayer::afterCaptured, this) ,path);
#endif
...
void FPSpecialResultLayer::afterCaptured(bool succeed, const std::string &outputFile)
{
    if (succeed) {
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
...
void FPSpecialResultLayer::onEnter(){
    Layer::onEnter();
...
void FPSpecialResultLayer::onExit(){
    Layer::onExit();
