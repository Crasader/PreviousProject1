...
public class SocketClient {
	...
	public SocketResponsePacket readDataToLength(final int length) {
		...
		if (getSocketConfigure().getSocketPacketHelper()
				.getReadStrategy() != SocketPacketHelper.ReadStrategy.Manually) {
			return null;
		}
			...
			public void run() {
				...
				try {
					...
					if (self.getSocketConfigure().getCharsetName() != null) {
						self.getReceivingResponsePacket()
								.buildStringWithCharsetName(self.getSocketConfigure().getCharsetName());
					}
	...
	/**
	 * 设置默认的编码格式 为null表示不自动转换data到string
	 */
	...
	/**
	 * 发送接收时对信息的处理 发送添加尾部信息 接收使用尾部信息截断消息
	 */
	...
	/**
	 * 当前连接状态 当设置状态为{@link State#Connected}, 收发线程等初始操作均未启动 此状态仅为一个标识
	 */
	private State state;
	...
	/**
	 * 记录上次发送数据片段的时间 仅在每个发送包开始发送时计时，结束后重置计时 NoSendingTime 表示当前没有在发送数据
	 */
	...
	private void __i__sendHeartBeat() {
		...
		if (getSocketConfigure() == null || getSocketConfigure().getHeartBeatHelper() == null
	...
	private void __i__onConnected() {
		...
		ArrayList<SocketClientDelegate> delegatesCopy = (ArrayList<SocketClientDelegate>) getSocketClientDelegates()
				.clone();
	...
	private void __i__onSendPacketBegin(final SocketPacket packet) {
		...
		if (getSocketClientDelegates().size() > 0) {
			ArrayList<SocketClientSendingDelegate> delegatesCopy = (ArrayList<SocketClientSendingDelegate>) getSocketClientSendingDelegates()
					.clone();
			int count = delegatesCopy.size();
			for (int i = 0; i < count; ++i) {
	...
	private void __i__onSendingPacketInProgress(final SocketPacket packet, final int sendedLength,
			final int headerLength, final int packetLengthDataLength, final int dataLength, final int trailerLength) {
		if (Looper.myLooper() != Looper.getMainLooper()) {
			getUiHandler().post(new Runnable() {
				@Override
				public void run() {
					self.__i__onSendingPacketInProgress(packet, sendedLength, headerLength, packetLengthDataLength,
	...
	private void __i__onReceivePacketBegin(final SocketResponsePacket packet) {
		...
		if (getSocketClientReceivingDelegates().size() > 0) {
			ArrayList<SocketClientReceivingDelegate> delegatesCopy = (ArrayList<SocketClientReceivingDelegate>) getSocketClientReceivingDelegates()
					.clone();
			int count = delegatesCopy.size();
			for (int i = 0; i < count; ++i) {
	...
	private void __i__onReceivingPacketInProgress(final SocketResponsePacket packet, final int receivedLength,
			final int headerLength, final int packetLengthDataLength, final int dataLength, final int trailerLength) {
		...
		if (Looper.myLooper() != Looper.getMainLooper()) {
			getUiHandler().post(new Runnable() {
				@Override
				public void run() {
					self.__i__onReceivingPacketInProgress(packet, receivedLength, headerLength, packetLengthDataLength,
							dataLength, trailerLength);
				}
			});
			return;
		...
		public void run() {
			...
			try {
				while (self.isConnected() && !Thread.interrupted()
					...
					if (packet.getData() == null && packet.getMessage() != null) {
						if (self.getSocketConfigure().getCharsetName() == null) {
					...
					byte[] packetLengthData = self.getSocketConfigure().getSocketPacketHelper()
							.getSendPacketLengthData(packet.getData().length + trailerDataLength);
					...
					self.__i__onSendPacketBegin(packet);
					self.__i__onSendingPacketInProgress(packet, sendedPacketLength, headerDataLength,
							packetLengthDataLength, packet.getData().length, trailerDataLength);
						...
						if (packet.getData().length > 0) {
							...
							if (self.getSocketConfigure().getSocketPacketHelper().isSendSegmentEnabled()) {
								segmentLength = Math.min(segmentLength,
										self.getSocketConfigure().getSocketPacketHelper().getSendSegmentLength());
							}
	...
	private class ReceiveThread extends Thread {
		...
		public void run() {
			...
			if (self.getSocketConfigure().getSocketPacketHelper()
					.getReadStrategy() == SocketPacketHelper.ReadStrategy.Manually) {
				return;
			}
			try {
				while (self.isConnected() && self.getSocketInputReader() != null && !Thread.interrupted()) {
					...
					int packetLengthDataLength = self.getSocketConfigure().getSocketPacketHelper()
							.getReceivePacketLengthDataLength();
					...
					if (self.getSocketConfigure().getSocketPacketHelper()
							.getReadStrategy() == SocketPacketHelper.ReadStrategy.AutoReadByLength) {
						if (packetLengthDataLength < 0) {
							self.__i__onReceivePacketCancel(packet);
							self.setReceivingResponsePacket(null);
						...
						int bodyTrailerLength = self.getSocketConfigure().getSocketPacketHelper()
								.getReceivePacketDataLength(data);
						...
						if (dataLength > 0) {
							...
							if (self.getSocketConfigure().getSocketPacketHelper().isReceiveSegmentEnabled()) {
								segmentLength = Math.min(segmentLength,
										self.getSocketConfigure().getSocketPacketHelper().getReceiveSegmentLength());
							}
							int offset = 0;
							while (offset < dataLength) {
								...
								self.__i__onReceivingPacketInProgress(packet, receivedPacketLength, headerDataLength,
										packetLengthDataLength, dataLength, trailerDataLength);
